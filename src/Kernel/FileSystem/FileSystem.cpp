...
namespace Kernel {
  ErrorOr<RefPtr<OpenFileDescriptor>> FileSystem::open(const Credentials& credentials, StringView path, FileOpenMode mode) {
    ...
    if(path_parts.size() == 0) {
      return TRY(m_open_files.create_descriptor_and_open(path, root_inode));
    }
    ...
    while(node_index < path_parts.size()) {
      ...
      if(!contains_path_part) {
        return Error::create_from_string("Path not found");
      }

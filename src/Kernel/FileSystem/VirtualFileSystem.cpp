...
namespace Kernel {
  ...
  ErrorOr<RefPtr<OpenFileDescriptor>> VirtualFileSystem::open(const Credentials& credentials, const String& path,
                                                              FileOpenMode mode) {
    auto path_parts = path.split("/");

    //This traverses all the mount points and finds the one with the longest common prefix
    const auto error_or_mount = longest_common_prefix(path_parts);
    ...
    auto [fs, longest] = error_or_mount.get_value();
    ...
    if(!fs) {
      return ErrorOr<RefPtr<OpenFileDescriptor>>::create_error(
    ...
    if(!fs->exists(relative_path)) {
      ...
      if(mode.is<FileOperationMode::Exclusive>()) {
        return ErrorOr<RefPtr<OpenFileDescriptor>>::create_error(
                Error::create_from_string("Cannot open file in exclusive mode! File already exists!"));
      }
    }

    const auto error_or_open = fs->open(credentials, relative_path, mode);
  ...
  ErrorOr<RefPtr<FileSystem>> VirtualFileSystem::device_load_filesystem(RefPtr<Device> device) {
    if(device->get_device_type() != DeviceType::Block) {
      return ErrorOr<RefPtr<FileSystem>>::create_error(
              Error::create_from_string("Device is not a block device!"));
    }
  ...
  ErrorOr<void> VirtualFileSystem::fs_mount(const StringView& mount_point, RefPtr<FileSystem> file_system,
                                            u64 mount_flags) {
    ...
    m_mounts.set(mount_point, RefPtr(new MountContext(move(file_system), mount_flags)));
  ...
  ErrorOr<Pair<RefPtr<FileSystem>, u32>> VirtualFileSystem::longest_common_prefix(const ArrayList<String>& path) {
    if(!m_root_inode) {
      return ErrorOr<Pair<RefPtr<FileSystem>, u32>>::create_error(
              Error::create_from_string("No root filesystem mounted!"));
    }
  ...
  ErrorOr<void> VirtualFileSystem::mount(const StringView& mount_point, const RefPtr<FileSystem>& file_system) {
    if(m_mounts.has_key(mount_point)) {
      return ErrorOr<void>::create_error(Error::create_from_string("A filesystem already mounted here!"));
    }

    m_mounts.set(mount_point, RefPtr(new MountContext(file_system, MountFlags::implicit())));

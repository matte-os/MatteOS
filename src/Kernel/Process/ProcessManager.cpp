...
ProcessManager::initialize_kernel_process(PageTable *kernel_root_table) {
  ...
  for (size_t i = 0; i < number_of_harts; i++) {
    threads->set(i, initialize_thread(reinterpret_cast<uintptr_t>(nullptr),
                                      CPU::read_satp(), i));
  }
  DebugConsole::println("Test");
  m_kernel_process =
      new KernelProcess(0, threads, kernel_root_table, ProcessState::RUNNING);
  return *m_kernel_process;
}

extern "C" void dummy_process_main() {
  for (;;) {
  }
}
...
PageTable *ProcessManager::create_dummy_process(uintptr_t text_start,
                                                uintptr_t text_end) {
  PageTable *root =
      reinterpret_cast<PageTable *>(MemoryManager::the().zalloc(1));

  MemoryManager::the().map_range(*root, text_start, text_end,
                                 (u64)(Memory::EntryBits::USER_READ_EXECUTE));
  auto satp =
      Kernel::CPU::build_satp(Kernel::SatpMode::Sv39, 1, (uintptr_t)root);
  ...
  MemoryManager::the().map(
      *root, ProcessManager::SPECIAL_ADDRESS,
...
Thread *ProcessManager::initialize_thread(uintptr_t program_counter, SATP satp,
  ...
  trap_frame->regs[2] = reinterpret_cast<u64>(trap_frame->trap_stack);
